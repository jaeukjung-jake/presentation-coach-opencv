...
bool LinearRegressor::learn(cv::Mat &data, cv::Mat &labels, bool isPCA)
{
    ...
    if(this->isPCA){
        ...
        cv::PCA FeaturePCA(mdata, cv::Mat(), PCA::DATA_AS_ROW);
        std::cout << "���������߶�: " <<FeaturePCA.eigenvectors.size() << std::endl;
        std::cout << "����ֵ�߶�:   " <<FeaturePCA.eigenvalues.size() << std::endl;
...
void ldmarkmodel::train(std::vector<ImageLabel> &mImageLabels){
    ...
    std::cout << "������ѵ����������������20��." << std::endl;
    std::cin >> samplesNum;
    if(samplesNum < 20)
        samplesNum = 20;
    else if(samplesNum > mImageLabels.size())
        samplesNum = mImageLabels.size();
    std::cout << "һ��" << samplesNum << "��ѵ������.\n" << std::endl;

    std::cout << "�Ƿ������۾����׼��һ��Landmarker����ƫ��?\n[Y/N?]" << std::endl;
    std::string t;
    std::cin >> t;
    if(t=="Y" || t=="y")
        isNormal = true;
...
//        cv::namedWindow("Image", cv::WINDOW_NORMAL);
    for(int i=0; i<samplesNum; i++){
        ...
        cv::Mat Image = cv::imread(mImageLabels.at(i).imagePath, IMREAD_GRAYSCALE); //CV_LOAD_IMAGE_GRAYSCALE
...
//            cv::waitKey(10);
    ...
    for(int i=0; i<samplesNum; i++){
        ...
        float distance = sqrt( (rx-lx)*(rx-lx)+(ry-ly)*(ry-ly) );//�������۵ľ���
        for(int j=0; j<numLandmarks; j++){
    ...
    for(int i=0; i<LinearRegressors.size(); i++){
        ...
        for(int j=0; j<samplesNum; j++){
            cv::Mat grayImage = cv::imread(mImageLabels.at(j).imagePath, IMREAD_GRAYSCALE);
...
void ldmarkmodel::printmodel(){
    if(isNormal)
        ...
        std::cout << "����һ������" << std::endl;
    std::cout << "һ��" << LinearRegressors.size() << "�ε����ع�..." << std::endl;
    for(int i=0; i<LandmarkIndexs.size(); i++){
        std::cout <<"��"<<i<<"�λع�: "<<LandmarkIndexs.at(i).size()<<"����  ";
...
void ldmarkmodel::drawPose(cv::Mat& img, const cv::Mat& current_shape, float lineL)
{
    ...
    cv::putText(img, txt,  cv::Point(60, 20), 0,0.5, cv::Scalar(0,0,255));
    std::stringstream ss1;
    ss1 << eav[1];
    std::string txt1 = "Yaw: " + ss1.str();
    cv::putText(img, txt1, cv::Point(60, 40), 0,0.5, cv::Scalar(0,0,255));
    std::stringstream ss2;
    ss2 << eav[2];
    std::string txt2 = "Roll: " + ss2.str();
    cv::putText(img, txt2, cv::Point(60, 60), 0,0.5, cv::Scalar(0,0,255));

    predict = tmp*estimateHeadPoseMat2;
    std::stringstream ss3;
    ss3 << predict.at<float>(0);
    txt = "Pitch: " + ss3.str();
    cv::putText(img, txt,  cv::Point(340, 20), 0,0.5, cv::Scalar(255,255,255));
    std::stringstream ss4;
    ss4 << predict.at<float>(1);
    txt1 = "Yaw: " + ss4.str();
    cv::putText(img, txt1, cv::Point(340, 40), 0,0.5, cv::Scalar(255,255,255));
    std::stringstream ss5;
    ss5 << predict.at<float>(2);
    txt2 = "Roll: " + ss5.str();
    cv::putText(img, txt2, cv::Point(340, 60), 0,0.5, cv::Scalar(255,255,255));
//        Pitch = eav[0];
//        Yaw	  = eav[1];
//        Roll  = eav[2];
}
